...
import { historicalDataService, type HistoricalPeriod } from '@/services/historicalDataService'
...
// Dados históricos serão carregados dinamicamente
...
export function SimulatorPage() {
  ...
  const [selectedHistoricalPeriod, setSelectedHistoricalPeriod] = useState<HistoricalPeriod | null>(null)
  ...
  // Carregar dados históricos ao montar o componente
  useEffect(() => {
    loadHistoricalData()
  }, [])
  ...
  const loadHistoricalData = async () => {
    ...
    try {
      const periods = await historicalDataService.getLatestPeriods()
      setHistoricalPeriods(periods)
      
      if (periods.length > 0) {
        setSelectedHistoricalPeriod(periods[0])
        // Definir assets padrão baseados no primeiro período
        const defaultAssets = Object.keys(periods[0].assets).slice(0, 4)
                ...
                <div className="space-y-2">
                  ...
                  {isLoadingHistorical ? (
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => {
                        const period = historicalPeriods.find(p => p.name === value)
                        if (period) {
                          setSelectedHistoricalPeriod(period)
                          // Atualizar assets disponíveis
                          const availableAssets = Object.keys(period.assets)
                          setSelectedAssets(availableAssets.slice(0, 4))
                        }
                      }}
                    >
                      <SelectTrigger className="bg-white">
                        ...
                        {historicalPeriods.map(period => (
                          <SelectItem key={period.id} value={period.name}>
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-purple-600">{period.duration}</span>
                            </div>
                          </SelectItem>
                        ))}
                      </SelectContent>
                ...
                {selectedHistoricalPeriod && (
                  <div className="p-3 bg-white/50 rounded-lg border border-purple-200">
                    <div className="text-xs text-purple-900 space-y-1">
                      ...
                      <div><strong>Data:</strong> {new Date(selectedHistoricalPeriod.startDate).toLocaleDateString('pt-BR')} - {new Date(selectedHistoricalPeriod.endDate).toLocaleDateString('pt-BR')}</div>
                    </div>
                  </div>
                )}
                      ...
                      {Object.entries(selectedHistoricalPeriod.assets).map(([key, asset]) => (
                        <label key={key} className="flex items-center space-x-2">
                          <input
                            ...
                            className="rounded"
                          />
                          <span className="text-xs text-purple-800">{asset.name}</span>
                        </label>
                      ))}
                    </div>
                  </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {selectedHistoricalPeriod ? (
                      ...
                      <Card key={assetKey} className={`${isPositive ? 'bg-gradient-to-br from-green-50 to-emerald-50 border-green-200' : 'bg-gradient-to-br from-red-50 to-rose-50 border-red-200'}`}>
                        <CardContent className="p-4">
                          <div className="text-center">
                            <div className="text-xs font-medium text-gray-600 mb-1">{asset.name}</div>
                ...
                <div className="text-center py-8">
                  <p className="text-gray-500">Selecione um período histórico para ver os resultados</p>
                </div>
              )}
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
                    ...
                    <span>Comparação de Performance - {selectedHistoricalPeriod?.name || 'Período'}</span>
                  </CardTitle>
                  <CardDescription>
                ...
                <CardContent>
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <ComposedChart 
                        data={selectedHistoricalPeriod ? selectedAssets.map(assetKey => {
                          const asset = selectedHistoricalPeriod.assets[assetKey]
                          if (!asset || asset.return === 0) return null
